...
async function drawBodyLines(pdfDoc, title, bodyText) {
...
function getDocumentMeta(documentType, data) {
  ...
  return {
    title: TITLE_BY_TYPE[documentType],
    prefix,
    year,
...
async function generatePayslipPdf(data) {
  const meta = getDocumentMeta("payslip", data);
...
async function generateSinglePdf(documentType, data) {
 ...
 * @param {object} data - Either { profile, input, calculation } for buildDocumentData, or pre-built document data
 * @returns {Promise<{blob: Blob, fileName: string}|Array<{blob: Blob, fileName: string}>>}
 *   - For "contract": array of [contractPdf, clausePdf]
 *   - For others: single { blob, fileName }
 */
export async function generatePDFClient(documentType, data) {
  const builtData = data.placeholders ? data : buildDocumentData(data);

  if (documentType === "payslip") {
    return generatePayslipPdf(builtData);
  }
  ...
  return generateSinglePdf(documentType, builtData);
}
